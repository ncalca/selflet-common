...
public class CollectionUtils {
	...
	public static <T extends IWeightedItem> T weightedRandomElement(Collection<T> collection) {
		...
		throw new IllegalStateException("Cannote extract random item from collection. Total weight: " + totalWeight);
	}
	...
	public static <T> void checkEmpty(final Collection<T> collection) {
		...
		if (collection.isEmpty()) {
			throw new IllegalArgumentException("Trying to extract a random element from an empty collection");
		}
	}

	public static <T extends IWeightedItem> void checkWeights(final Collection<T> collection) {
		for (T item : collection) {
			if (item.weight() < 0) {
				throw new IllegalArgumentException("Weight of item cannot be zero or negative: " + item);
			}
		}
	}
