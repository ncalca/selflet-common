...
/**
 * This piece of code is extracted from the Protopeer simulator
 * 
 ...
 * 
 * <br/>
 * Note that the <code>Configuration</code> class itself does not contain any
 * public non-static fields and needs to be subclassed to be useful.
 * 
 */
public class Configuration {
	...
	/**
	 * This function is called whenever the <code>Configuration</code> needs to
	 * create a concrete instance of a <code>fieldType</code> from a string
	 ...
	 * <code>java.net.InetAddress</code> and all <code>enum</code>s. Subclasses
	 * can override this method to add support for additional
	 * <code>fieldType</code>s.
	 * 
	 * @param value
		...
		try {
			if (fieldType.isEnum()) {
				...
				if (!value.equals("true") && !value.equals("false")) {
					throw new ConfigurationException("The boolean field: " + field.getName() + " should either be 'true' or 'false', it is: " + value);
				}
				field.set(this, new Boolean(value.equals("true")));
			} else {
				throw new ConfigurationException("Unsupported type " + fieldType.getCanonicalName() + " in params loader, field name: " + field.getName()
						+ " read value: " + value);
			}
	 ...
	 * 
	 * @param properties
	 *            the <code>Properties</code> object that contains the
	 *            (<b>key</b>, <b>value</b>) pairs with which this
	 ...
	 *            <code>properties</code> argument and the public static fields
	 *            in this object, otherwise an
	...
	public void loadFromProperties(Properties properties) throws ConfigurationException {
		...
		for (Map.Entry<Object, Object> entry : properties.entrySet()) {
			...
			try {
				...
				throw new ConfigurationException("Field not found in the " + this.getClass().getCanonicalName() + " class: " + key);
			}
		...
		if (!fields.isEmpty()) {
			StringBuffer uninitializedFields = new StringBuffer();
			for (Field field : fields) {
				uninitializedFields.append(field.getName() + " ");
			}
